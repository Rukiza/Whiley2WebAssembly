...
//    public static void execWasmWriter(String wyilDir, Path.ID id) throws IOException {
//
//    }
    ...
    public static String execWast(String commandLocation, String srcDir, String fileName, String... args) {
        try {
            srcDir = srcDir.replace('/', File.separatorChar);
            commandLocation = commandLocation.replace('/', File.separatorChar);
            String tmp = commandLocation+"wasm -t "+ fileName;
            Process p = Runtime.getRuntime().exec(tmp, null, new File(srcDir));
